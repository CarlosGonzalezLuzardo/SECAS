...
class UsernamePasswordMako(UserAuthnMethod):
    ...
    def verify(self, request, **kwargs):
        ...
        try:
            verify_user = UserManager.verify_username(_dict["login"][0])
            verify_option = 1
        except Exception:
            verify_user = UserManager.verify_username(_dict["username"][0])
            verify_option = 2
        ...
        if verify_user:
            ...
            try:
                if (verify_option==1):
                    ...
                    self._verify(_dict["password"][0], _dict["username"][0])
                ...
                kwargs["query"] = _dict["query"][0]
                argv = self.templ_arg_func(0, **kwargs)
                argv['wrong_value'] = 1
                kwargs["path"] = _dict["url"][0]
                kwargs["requrl"] = _dict["url"][0]
                kwargs["url"] = _dict["url"][0]
                ##kwargs["path"] = kwargs["query"]
                argv['action'] = _dict["url"][0]
                logger.info("do_authentication argv: %s" % sanitize(argv))
                mte = self.template_lookup.get_template(self.mako_template)
